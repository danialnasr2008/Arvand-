                ...
                <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4 text-right shadow-inner" id="post-crud-form">
                  ...
                  <form onSubmit={handlePostSubmit} className="space-y-4" id="crud-form-element">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" id="post-form-row-2">
                      ...
                      <div id="form-field-image" className={`${formPostType === 'news' ? 'sm:col-span-2' : ''} space-y-2`}>
                        <div className="flex items-center justify-between">
                          <label className="block text-xs font-bold text-slate-600 dark:text-slate-400">تصویر شاخص</label>
                          <div className="flex gap-1.5 bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-150 dark:border-slate-800/80">
                            <button
                              type="button"
                              onClick={() => setImageInputMode('upload')}
                              className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${imageInputMode === 'upload' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}
                            >
                              آپلود فایل عکس
                            </button>
                            <button
                              ...
                              className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${imageInputMode === 'url' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}
                            >
                              لینک تصویر (URL)
                            </button>
                          </div>
                        ...
                        {imageInputMode === 'upload' ? (
                          <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-4 bg-slate-50 dark:bg-slate-950/20 hover:bg-slate-100/40 dark:hover:bg-slate-950/40 transition-colors relative group min-h-[140px]">
                            {formImageUrl && formImageUrl.startsWith('data:') ? (
                              <div className="relative w-full flex flex-col items-center gap-2">
                                <img
                                  src={formImageUrl}
                                  alt="Preview"
                                  className="h-28 w-auto object-cover rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm"
                                />
                                <button
                                  type="button"
                                  onClick={() => setFormImageUrl('')}
                                  className="text-[10px] font-bold bg-rose-50 hover:bg-rose-100 text-rose-600 px-3 py-1.5 rounded-lg dark:bg-rose-950/30 dark:hover:bg-rose-900/40 transition-colors cursor-pointer"
                                >
                                  حذف عکس و انتخاب مجدد
                                </button>
                              </div>
                            ) : (
                              <label className="cursor-pointer w-full py-4 text-center block space-y-1.5">
                                <div className="flex justify-center text-slate-400 group-hover:text-blue-500 transition-colors">
                                  <Upload className="h-6 w-6" />
                                </div>
                                <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 block">برای بارگذاری تصویر کلیک کنید یا عکس را به اینجا بکشید</span>
                                <span className="text-[10px] text-slate-400 block">حداکثر حجم ۲ مگابایت (فرمت‌های JPG, PNG, WEBP)</span>
                                <input
                                  type="file"
                                  accept="image/*"
                                  onChange={(e) => {
                          ...
                          <div className="space-y-2">
                            ...
                            />
                            {formImageUrl && (
                              <div className="flex items-center gap-3 p-2 bg-slate-50 dark:bg-slate-950/20 border border-slate-150 dark:border-slate-800/60 rounded-xl">
                                <img
                                  src={formImageUrl}
                                  alt="Preview"
                                  className="h-10 w-16 object-cover rounded-lg border border-slate-200 dark:border-slate-800 shrink-0"
                                  onError={(e) => {
                                    (e.target as HTMLElement).style.display = 'none';
                                  }}
                                />
                                <span className="text-[10px] text-slate-400 font-medium">پیش‌نمایش تصویر متصل شده</span>
                              </div>
                            )}
                          </div>
                        )}
